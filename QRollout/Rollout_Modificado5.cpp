...
TJob *QRolloutThread::Rollout_Modificato5 ( int force )
{
    ...
    while ( cont_livelli > 0 )
    {
        ...
        if ( cont_livelli == GNum_Job-1 ) //
        {
            ...
            tempo_massimo_di_completamento = Schedula::MaxCmaxSchedula ( vett_schedule_parziali,GNum_Macchine );
        ...
        else
        {
            ...
            StampaPermutazioni ( Fpermutazione_Buffer,GNum_Job );
            pp++;
        ...
        for ( iter_for=0;iter_for<GNum_Job;iter_for++ )
        {
            ...
            else //se e' selezionabile
            {
                ...
                for ( i = 0;i<Fnum_Heur;i++ ) //num_heur contiene il numero di heuristiche effettivamente usato
                {
                    ...
                    Schedula::AzzeraSchedule();
                    //calcolo tramite una euristica la parte mancante della permutazione
                    ...
                    SalvaSeMeglio ( perm_di_passaggio );//qui modifica permutazione_buffer
                    delete[] perm_di_passaggio;

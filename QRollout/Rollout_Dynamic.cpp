...
*/
TJob *QRolloutThread::Rollout_Dynamic ( int pForce )
{
    ...
    TInsieme_Dinamico *insieme;
    TInsieme_Dinamico *insieme_next;
    TInsieme_Dinamico *insieme_temp;
            ...
            else //se e' selezionabile
            {
                ...
                for(kk = 0;kk < cont_livelli; kk++)
                {
                    while(array_job_attuale[iter].ID == -1 ||
                          array_job_attuale[iter].ID== insieme_temp->ID)
                        iter++;
